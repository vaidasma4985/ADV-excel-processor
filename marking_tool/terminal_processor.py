...
def process_terminal_result(file_bytes: bytes, file_name: str) -> dict[str, Any]:
    """Run the preserved terminal workflow and package its workbook/debug outputs."""
    terminal_debug_messages: list[str] = []
    terminal_df, user_info_messages, developer_debug_messages = parse_terminal_input(file_bytes)
    terminal_debug_messages.extend(developer_debug_messages)
...
def _write_markings_block_header(
    ...
    *,
    ...
    """Write one two-row block header above one markings export block."""
...
def _write_terminal_markings_sheet(
    ...
    _write_markings_block_header(
        ...
        startrow=0,
        startcol=0,
        block_width=max(1, len(terminal_tmb_export_df.columns)),
        title="TERMINALS TMB",
    ...
    _write_markings_block_header(
        ...
        block_width=max(1, len(terminal_strip_export_df.columns)),
        title="TERMINALS STRIP",
        subtitle=_TERMINAL_STRIP_BLOCK_SUBTITLE,
    )
    terminal_tmb_export_df.to_excel(
        writer,
    ...
    terminal_strip_export_df.to_excel(
        writer,
        sheet_name=sheet_name,
        index=False,
        startrow=data_start_row,
...
def _write_wire_markings_sheet(
    ...
    """Write the placeholder Cable Marking sheet with CABLES and POWER WIRES blocks."""
