...
def compute_feeder_paths(
    ...
    main_root_nets = {
        node for node in net_nodes if _MAIN_ROOT_PATTERN.match(_net_name(node))
    }
    sub_root_nets = sorted(
        _net_name(node)
        for node in net_nodes
        if _SUB_ROOT_PATTERN.match(_net_name(node))
    )
    ...
    debug = {
        ...
        "main_root_nets_count": len(main_root_nets),
